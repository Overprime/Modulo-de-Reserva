...
<div class="col-md-4 column">
<label for="">OT:</label>
<input type="text" class="form-control"name="ot" value="<?php echo $Ot;?>" readonly>
</div>
</div>
<div class="row clearfix">
<div class="col-md-4 column">
...
<select name="cencos" id="" class="form-control" required  READDONLY>
<?php
$link=Conectarse();
$Sql="SELECT  CODIGOCENTROCOSTO AS COD_CO,CODIGOOT AS OT ,
CENCOST_DESCRIPCION AS DESC_OT FROM 
[020BDCOMUN].DBO.CENCOSOT AS CO  INNER JOIN 
[011BDCONTABILIDAD].DBO.CENTRO_COSTOS AS CC  
ON CO.CODIGOCENTROCOSTO=CC.CENCOST_CODIGO WHERE CODIGOOT='$Ot'
";
$result=mssql_query($Sql) or die(mssql_error());
while ($row=mssql_fetch_array($result)) {
?>
<option value="<?php echo $row['COD_CO']?>">
<?php echo $row['COD_CO'].' - '.
utf8_encode($row['DESC_OT'])?></option>
<?php }?>
</select>
</div>
</div>
...
datos que ha  ingresado previamente?
</div>
